...
export function buildIsoSVG(model, byRoom, hiddenEids, focusZ, floorGap, horizGap, lightsByEid={}, lightsLoading=false, floors=[], opts={}){
  ...
  const TEMP_WARM_AT=20;
  const TEMP_WARM_WASH="#ef4444", TEMP_COOL_WASH="#3b82f6";
  const TEMP_WARM_INK="#fca5a5",  TEMP_COOL_INK="#93c5fd";
  const mixHex=(a,b,t)=>{
  ...
  for(const z of levels){
    ...
    const markerSvg=(l,hx,hy,entry,extra="",suppressGlyph=false)=>{
      ...
      const tempWarm=tempLive && Number(l.temperature)>=TEMP_WARM_AT;
      ...
      const tempWash=tempLive ? layer(`fill="${tempWarm?TEMP_WARM_WASH:TEMP_COOL_WASH}" fill-opacity="0.30" stroke="none" pointer-events="none"`) : "";
      ...
      const tempLbl=tempLive
        ? `<text x="${hx.toFixed(1)}" y="${hy.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" `+
          ...
          `fill="${tempWarm?TEMP_WARM_INK:TEMP_COOL_INK}" paint-order="stroke" stroke="#050d09" stroke-width="${(TEMP_DIGIT_PX*0.32).toFixed(1)}" `+
